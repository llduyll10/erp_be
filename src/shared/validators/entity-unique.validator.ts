...
export class EntityUniqueValidator implements ValidatorConstraintInterface {
  ...
  async validate<E>(value: any, args: UniqueValidationArguments<E>) {
    ...
    const query = {
      [args.property]: value,
      ...(args.object[primaryKey as keyof typeof args.object]
        ? { [primaryKey]: Not(args.object[primaryKey as keyof typeof args.object]) }
        : {}
      ),
  ...
  defaultMessage<E>(args: UniqueValidationArguments<E>) {
    return `A ${this.dataSource.getRepository(args.constraints[0]).metadata.tableName
      } with this ${args.property} already exists`;
  }
}
