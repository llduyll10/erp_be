...
@EventSubscriber()
@Injectable()
export class StorageFileSubscriber
  implements EntitySubscriberInterface<StorageFile> {
  constructor(
  ...
  async afterLoad(entity: StorageFile): Promise<void> {
    ...
    if (entity.disk === DriverType.S3) {
      const signedUrlResponse = await storageDisk.getSignedUrl(
        entity.file_path,
      );
      entity.url = signedUrlResponse.signedUrl;
    } else {
      entity.url = `${this.configService.get(
        'storage.disks.local.config.publicUrl',
      )}/${entity.file_path}`;
    }
  }
}
